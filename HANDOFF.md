...
- 表白墙使用 Three.js 粉色爱心场景，爱心粒子以便签形式呈现；游客/普通 `user` 便签初次提交待审，三种管理角色便签免审并立即公开；
...
- `/confessions` 公开可访问和匿名提交；当前专用页面限制便签最多 280 字，并提交固定 `#表白`、`anonymous=true`。游客和普通 `user` 初次提交时创建为 `pending + pending` 并进入全局审核队列/outbox；`reviewer/admin/super_admin` 初次提交时直接为 `visible + approved`，不入队；
...
2. 新建时游客/普通 `user` 的普通校园动态和表白便签进入全局审核队列；管理角色的普通动态/表白便签及所有登录用户的失物招领初始免审。任何内容被管理端明确“退回待审”后也会进入同一队列。
...
├─ 表白墙便签
│  ├─ 游客或普通 user → pending + pending → outbox/全局队列
...
后端支持飞书自定义群机器人和企业微信群机器人，可单独或同时启用。游客/普通 `user` 的普通校园动态或表白便签初次进入待审，或任意内容被管理端明确退回待审时，系统才将任务写入 PostgreSQL `moderation_notification_outbox`，再由后台 worker 异步发送。管理角色普通动态/表白便签和登录用户失物招领的初次免审发布不产生审核提醒。
...
- [ ] 审核矩阵正确：游客/普通用户的普通动态与表白便签初始待审；三种管理角色的普通动态/表白便签和所有登录用户的失物招领初始免审；任意内容被明确退回后带 `review_hold` 入队且作者编辑不能自行公开；所有审核员对实际待审队列完全同权；
