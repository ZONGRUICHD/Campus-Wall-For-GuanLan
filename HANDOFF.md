...
- 用户可用任意合规用户名与密码注册；
- 游客与普通 `user` 初次发布的普通校园动态先进入统一待审核队列；`reviewer/admin/super_admin` 初次发布的普通动态立即公开，不进入队列；
- 表白墙使用 Three.js 粉色爱心场景，爱心粒子以便签形式呈现；所有可发布者初次提交的便签立即公开、无需审核；
- 首页持续展示最近发布或编辑的校园公告；审核员、管理员和超级管理员均可发布、编辑或收回；
- 失物招领必须登录后才能查看和发布，初次发布后立即对登录用户可见，不进入审核队列；
- 登录用户可维护昵称、头像、简介，查看自己的帖子、评论、收藏和通知；
- 管理角色也能发帖且普通动态直接公开；所有审核员可审核全局队列中的全部实际待审内容；
- 任意内容被管理端明确退回待审后会设置 `review_hold` 并进入全局队列；作者编辑不能自行重新公开；
...
| `/confessions` | Three.js 表白墙便签 | 公开浏览和发布；初次发布立即公开、不进入审核队列 |
...
- `/confessions` 公开可访问和匿名提交；当前专用页面限制便签最多 280 字，并提交固定 `#表白`、`anonymous=true`。所有可发布者初次提交的表白便签直接创建为 `visible + approved`，不进入审核队列或审核通知 outbox；
- 页面一次读取最近 72 条表白数据，但仍只展示 `moderation_status=visible`、`review_status=approved` 且正文非空的记录。免审只决定初始状态，下架、删除或其他非公开状态内容不得进入 Three.js 场景；
...
- 表白墙没有实时推送；本端发布成功后会立即重新拉取便签，因此无需手工刷新。其他设备仍在下一次加载时获得新内容。调整抓取数量、轮播顺序或随机算法时必须保留“按时间播放”和 reduced-motion 语义。
...
| `reviewer` | 普通动态立即公开 | 有，顶部导航直接显示 | 所有审核员完全相同；可审核统一队列中的全部实际待审内容并管理主页公告；不能管理用户、设置或角色 |
...
2. 新建时只有游客/普通 `user` 的普通校园动态进入全局审核队列；表白、失物招领及管理角色普通动态初始免审。任何内容被管理端明确“退回待审”后也会进入同一队列。
3. `reviewer` 可以审核队列中的全部帖子，不按作者、标签或负责范围过滤；该口径对单条和批量审核一致，操作仍写入审计。
...
├─ 表白墙便签（所有可发布者） → visible + approved（免审、不入队）
└─ 失物招领（必须登录）       → visible + approved（免审、不入队）

公开读取仍统一过滤：只返回 visible + approved

任意内容被管理端明确退回待审
└─ pending + pending + review_hold=true → 全局待审队列
...
- 游客与普通 `user` 的普通校园动态必须进入 `pending + pending`，不能冒充管理角色绕过审核；
- 未带 `review_hold` 的新建内容中，管理角色普通动态、表白便签和已登录用户的失物招领必须直接使用 `visible + approved`，且初次发布不写审核队列或审核通知 outbox；
- 任意内容被管理端明确退回时必须设置 `review_hold=true` 并进入全局待审队列/outbox；作者后续编辑不能清除该标记或自行恢复公开，只有审核员通过才能解除；
...
后端支持飞书自定义群机器人和企业微信群机器人，可单独或同时启用。游客/普通 `user` 的普通校园动态初次进入待审，或任意内容被管理端明确退回待审时，系统才将任务写入 PostgreSQL `moderation_notification_outbox`，再由后台 worker 异步发送。管理角色普通动态、表白便签和失物招领的初次免审发布不产生审核提醒。
...
4. 表白便签对游客及登录用户均立即公开，初次发布不进入队列/outbox，提交端会立即重新拉取；下架或删除后不再公开；
5. 失物招领未登录会跳转登录，登录后初次发布立即可见且不进入队列/outbox；匿名用户仍不能读取或猜附件地址；
6. 将管理角色普通动态、表白或失物招领明确退回待审后会设置 `review_hold` 并进入队列；作者编辑后仍不公开，审核员通过后才恢复；
7. 合法用户名的 2/24 字符边界、非法字符、NFKC 与大小写唯一性，以及 8/128 字符密码边界均符合第 7 节；
8. 四种角色的导航和后端权限一致；
9. 任意审核员能看到并处理全局队列中的全部实际待审内容；
10. 单条审核和批量审核均不因发布者身份产生额外限制，且审计记录完整；
11. 公开列表、详情、分区和互动接口始终只返回 `visible + approved`，pending/hidden/deleted 均不泄漏；
12. 超级管理员能分配角色，审核员和管理员不能；
13. 头像横图、竖图、透明 PNG、带方向信息图片均输出方形 WebP；
14. reviewer/admin/super_admin 都能发布、编辑、收回公告，普通用户不能访问管理接口；
15. 公告 GET/旧 POST 排序、公开字段脱敏、旧 ID 迁移、编辑后未读、换行和收回均符合第 9 节；
16. 公告中的 HTML 标签与事件属性只显示为文本，不在 React 页面执行；
17. 桌面和手机导航、模态框、表单及页面切换动效正常；
18. 深色、浅色、跟随系统和 `prefers-reduced-motion` 均正常；
19. 浏览器控制台无新增 warning/error，页面无 Vite 错误遮罩。
...
1. 顶部导航出现后台入口；
2. 进入统一帖子审核队列；
3. 可查看全局队列中的所有实际待审内容，包括被管理端设置 `review_hold` 后退回的免审类别，不需要手工输入 `/admin`；
...
- [ ] 审核矩阵正确：游客/普通用户普通动态初始待审；三种管理角色普通动态、所有表白便签和登录用户失物招领初始免审；任意内容被明确退回后带 `review_hold` 入队且作者编辑不能自行公开；所有审核员对实际待审队列完全同权；
