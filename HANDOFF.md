...
- 新上传的 JPEG、PNG、GIF、WebP 在服务器按真实内容解码并统一输出 WebP：自动纠正 EXIF、移除元数据，最长边默认 2048、初始质量 80、主图硬上限 1.5 MiB；透明 PNG 保留透明通道，GIF/动画 WebP 只保留第一帧，超过 200 帧拒绝。缩略图最长边 320、质量 68、目标不超过 160 KiB。直传和分块合并共用同一处理链；成功后只保留压缩主图和缩略图，原始图片与已完成分片立即删除，失败也必须清理临时文件。非图片附件与视频保持原逻辑；该规则只影响新上传，历史文件不会自动转换或删除；
