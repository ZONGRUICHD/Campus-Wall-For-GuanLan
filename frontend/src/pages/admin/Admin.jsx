...
const emptyStats = {
  ...
  users: { total: 0, active: 0, disabled: 0, muted: 0 },
  apps: { total: 0, published: 0, hidden: 0 },
...
export default function Admin() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const [response, session] = await Promise.all([api.adminGetDashboardStats(), api.adminVerify()])
      setStats({ ...emptyStats, ...(response.data?.stats || {}) })
      setPermissionNames((session.data?.admin?.permissions || []).map((permission) => permission.name))
  ...
  return (
    <AdminShell title="仪表盘">
      ...
      <div className="admin-overview-grid mt-5">
        <Metric icon="bi-chat-quote" label="公开留言" value={stats.messages.visible} detail={`今日新增 ${stats.messages.last_24_hours}，累计 ${stats.messages.total}`} />
        <Metric icon="bi-people" label="学生账号" value={stats.users.active} detail={`${stats.users.muted} 个禁言，${stats.users.disabled} 个停用`} tone="success" />
        <Metric icon="bi-flag" label="待处理举报" value={stats.reports.total} detail={`近 7 天处理 ${stats.reports.processed_last_7_days || 0} 条，累计 ${stats.reports.processed_total || 0} 条`} tone={stats.reports.total ? 'danger' : 'success'} />
        <Metric icon="bi-grid" label="展示中应用" value={stats.apps.published} detail={`${stats.apps.hidden} 个已下架，累计 ${stats.apps.total}`} tone="warning" />
      ...
      <div className="admin-dashboard-columns mt-6">
        ...
        <section className="admin-dashboard-section">
          <div className="admin-section-heading">
            ...
            {can('manage_wall_message') ? <Link className="btn btn-sm btn-outline" to="/admin/wall">进入管理</Link> : null}
          </div>
      ...
      <section className="admin-dashboard-section mt-6">
        <div className="admin-section-heading">
          <div>
            <h2>热门分区</h2>
            <p>按当前公开留言数量统计</p>
      ...
      <nav className="admin-quick-links mt-6" aria-label="管理快捷入口">
        {can('manage_wall_message') ? <Link to="/admin/wall"><i className="bi bi-chat-quote" /><span>留言管理</span><b>{stats.messages.pending_review}</b></Link> : null}
        ...
        {can('manage_users', 'manage_wall_message') ? <Link to="/admin/users"><i className="bi bi-people" /><span>用户管理</span><b>{stats.users.total}</b></Link> : null}
        ...
        {can('manage_apps') ? <Link to="/admin/apps"><i className="bi bi-grid" /><span>应用管理</span><b>{stats.apps.total}</b></Link> : null}
