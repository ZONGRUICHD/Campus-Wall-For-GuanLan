...
function BoundUser({ message }) {
  if (!message?.user_id) return <span className="text-muted">游客/旧留言</span>
  return (
    <div className="flex flex-wrap gap-2 text-sm">
      <span className="badge">{message.anonymous === false ? '非匿名' : '匿名'}</span>
      <span className="badge">学号：{message.user?.username || message.username || '-'}</span>
      <span className="badge">姓名：{message.user?.real_name || '-'}</span>
...
export default function AdminWall() {
  ...
  const allPageSelected = messages.length > 0 && messages.every((message) => selectedIds.includes(message.id))
  ...
  const togglePageSelection = () => {
    setSelectedIds(allPageSelected ? [] : messages.map((message) => message.id))
  }
  ...
  return (
    <AdminShell title="留言管理">
          ...
          <article className={`card message-card p-4 ${message.moderation_status === 'hidden' ? 'admin-message-hidden' : ''}`} key={message.id}>
            <div className="flex flex-col gap-3 md:flex-row md:justify-between">
              <div className="flex min-w-0 flex-1 gap-3">
                <input className="mt-1 h-5 w-5 shrink-0" type="checkbox" checked={selectedIds.includes(message.id)} onChange={() => toggleSelection(message.id)} aria-label={`选择留言 ${message.id}`} />
              ...
              <div className="grid grid-cols-2 gap-2 self-start md:w-48">
                <button className={`btn btn-sm justify-center ${message.review_status === 'approved' ? 'btn-outline' : 'btn-success'}`} disabled={busy} onClick={() => review(message, message.review_status === 'approved' ? 'return' : 'approve')}>
      ...
      <Modal visible={Boolean(selected)} title={`留言详情 #${selected?.id || ''}`} width="900px" onClose={() => setSelected(null)}>
        {selected ? (
          <div className="space-y-3">
            <div className="flex flex-wrap gap-2"><ReviewBadge message={selected} />{selected.moderation_status === 'hidden' ? <span className="badge status-danger">已下架</span> : null}</div>
            <div className="info-callout p-3"><div className="mb-2 font-bold">绑定用户</div><BoundUser message={selected} /></div>
